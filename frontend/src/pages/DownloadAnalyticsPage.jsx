...
import { Card, DatePicker, Select, Button, Form, Row, Col, Radio, Alert, Space, notification } from 'antd';
import { DownloadOutlined, FilePdfOutlined, FileExcelOutlined, FileTextOutlined, MailOutlined } from '@ant-design/icons';
import { downloadAnalyticsReport } from '../api/django-api';
  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      const formattedValues = {
        ...
        included_data: values.includedData ? values.includedData.join(',') : null,
        content_type: values.contentType,
        options: values.options ? values.options.join(',') : null
      };

      // Call the API to generate and download the report
      const response = await downloadAnalyticsReport(formattedValues);
      
      // Create a blob from the response data and trigger download
      const contentType = values.format === 'pdf' ? 'application/pdf' :
      ...
      const fileName = `analytics-report-${new Date().toISOString().slice(0, 10)}.${values.format}`;
  ...
  const handleEmailReport = async () => {
    try {
      ...
      if (!values.reportType || !values.dateRange) {
        api.warning({
          message: 'Warning',
          description: 'Please fill in required fields before emailing the report',
        });
        return;
      }
      ...
      // Call the API to email the report
      await downloadAnalyticsReport(formattedValues);
      
      api.success({
        message: 'Success',
        description: 'Analytics report will be emailed to you shortly!',
      });
    } catch (error) {
      console.error('Error emailing report:', error);
      api.error({
        message: 'Error',
        description: 'Failed to email analytics report',
      });
    }
  };
